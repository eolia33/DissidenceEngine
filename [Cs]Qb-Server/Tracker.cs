...
namespace Server
{
    public class Tracker : BaseScript
    {
        ...
        public Dictionary<string, Configuration.GpsDic> gpsListing { get; set; }
        public Dictionary<string, PlayerNoSql> playersNoSql { get; set; }
        public List<string> frequencyList { get; set; }
        public Tracker(Config _config,BridgeQBCore _bridgeQBcore, List<string> _frequencyList, Dictionary<string, Configuration.GpsDic> _gpsListing)
        {
            config = _config;
        ...
        {
            _ = Task.Run(() => GpsListeningClients());
        }

        public Task GpsListeningClients()
        {
            ...
            while (true)
            {
                ...
                foreach (var frequency in frequencyList)
                {
                    ...
                    var linqC = gpsListing.Where(x => x.Value.PedFrequency == frequency).Count();

                
                        var linq = gpsListing.Where(x => x.Value.PedFrequency == frequency);

                        Dictionary<string, Configuration.GpsDic> GpsListingJson = new Dictionary<string, Configuration.GpsDic>();
                        ...
                        var linqJson = gpsListing.Where(x => x.Value.PedFrequency == frequency);
       ...
       public bool isItTheLastManStanding(string frequency, string pedId)
        ...
        #region Gps Core
        public Configuration.GpsDic dictionaryConstruct(string licence, string id, string name, string frequency, Vector3 vector, string color, int gpsType, float direction)
        {
            var newGpsClient = new Configuration.GpsDic
            {
                PedLicence = licence,
                PedId = id,
                PedName = name,
